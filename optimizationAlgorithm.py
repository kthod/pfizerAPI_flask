...
class optimizationAlgorithm(ABC):
    ...
    def add_noise(self,
                ...
                p0_1=0.3):
        """
        Adds a noise model that includes:
        1) Depolarizing errors on 1-qubit and 2-qubit gates
        2) Thermal relaxation errors on 1-qubit and 2-qubit gates
        3) Readout errors on measurement
        
        Parameters:
            t1, t2 (float): T1/T2 times in microseconds (or nanoseconds, depending on your usage).
            prob_1_qubit (float): Depolarizing probability for 1-qubit gates.
            prob_2_qubit (float): Depolarizing probability for 2-qubit gates.
            p1_0, p0_1 (float): Readout error probabilities.
        """

        # Create a new noise model
        self.noise_model = NoiseModel()

        # Example gate time (in ns), just as before
        gate_time = 50

        # Create thermal relaxation errors for single qubit
        thermal_relaxation_qubit1 = thermal_relaxation_error(
            t1, t2, gate_time
        )
        thermal_relaxation_qubit2 = thermal_relaxation_error(
            t1, t2, gate_time
        )

        # Create a composite error for the two-qubit 'cx' gate
        # Tensor them together: first qubit error ⊗ second qubit error
        thermal_relaxation_cx = QuantumError.tensor(
            thermal_relaxation_qubit2, thermal_relaxation_qubit1
        )

        # Depolarizing errors
        depolarizing_error_1q = depolarizing_error(prob_1_qubit, 1)
        depolarizing_error_2q = depolarizing_error(prob_2_qubit, 2)

        # Readout error
        # Matrix format: 
        #     [
        #       [P(measure 0|state 0), P(measure 1|state 0)],
        #       [P(measure 0|state 1), P(measure 1|state 1)]
        #     ]
        # So p0_1 is "0 -> 1" error; p1_0 is "1 -> 0" error
        ...
        # ----------------------------------------------------------------
        # 1) Add depolarizing and thermal relaxation for *single-qubit* gates
        # ----------------------------------------------------------------
        #   In modern Qiskit, "u" is often the parameterized single-qubit gate 
        #   used internally. If you want to target all single-qubit gates 
        #   recognized by the transpiler, you could also list them individually 
        #   (e.g., ['x', 'sx', 'rz'])—just ensure that matches your basis set.
        # ----------------------------------------------------------------
        self.noise_model.add_all_qubit_quantum_error(
            depolarizing_error_1q, ['u']
        )
        self.noise_model.add_all_qubit_quantum_error(
            thermal_relaxation_qubit1, ['u']
        )

        # ----------------------------------------------------------------
        # 2) Add depolarizing and thermal relaxation for *two-qubit* 'cx' gates
        # ----------------------------------------------------------------
        self.noise_model.add_all_qubit_quantum_error(
            depolarizing_error_2q, ['cx']
        )
        self.noise_model.add_all_qubit_quantum_error(
            thermal_relaxation_cx, ['cx']
        )

        # ----------------------------------------------------------------
        # 3) Add readout error to measurement on all qubits
        # ----------------------------------------------------------------
        self.noise_model.add_all_qubit_readout_error(readout_error)

    def clear_noise(self):
        """
        Clears the noise model, reverting to an ideal simulator.
        """
        self.noise_model = None
...
#         self.noise_model = None
    ...
    def cost_function(self, parameters, number_of_measurements):
        ...
        job = simulator.run(transpiled_circ, shots=number_of_measurements)

            # Get the counts (measurement results)
        counts = job.result().get_counts()
        #statevector = job.result().get_statevector(transpiled_circ)
    ...
    def optimize(self, n_measurements=10000, number_of_experiments=10, maxiter=150, initial_vector = [],init_state='0' ):
        """
        ...
        for k in range(number_of_experiments):
            ...
            print(f"initial vec {initial_vector}")
        ...
        #np.save('toy_example_cost_evolution.npy', self.cost_evolution)
        ...
        print(f"ind {ind} opt cost {self.optimal_cost}")
