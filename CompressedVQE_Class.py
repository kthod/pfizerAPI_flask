...
class CompressedVQE(optimizationAlgorithm):
    ...
    def __init__(self, qubo_matrix: np.ndarray, layers: int = 1, na: int = 1) -> None:
        ...
        for i in range(self.nc):
            for j in range(i, self.nc):
                if qubo_matrix[i][j]==0 and qubo_matrix[j][i]!=0:
                    qubo_matrix[i][j] = qubo_matrix[j][i]
                    qubo_matrix[j][i] = 0
    ...
    def show_solution(self, shots):
        ...
        transpiled_circ = transpile(vqa_circ, simulator)
    ...
    def calculate_probabilities(self, counts):
        ...
        num_registers = int(np.ceil(nc / ancilla_len))
    ...
    def compute_expectation(self, counts: dict) -> float:
        ...
        for bitstring, count in counts.items():
            ...
            for i in range(na):
                for j in range(i, na):
                    ind_i = min(reg * na + i, nc - 1)
                    ind_j = min(reg * na + j, nc - 1)
                    if aux[i] == '1' and aux[j] == '1':
                        P[ind_i][ind_j] += count
                    register_counts[ind_i][ind_j] += count
